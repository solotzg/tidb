...
func (s *session) ExecuteStmt(ctx context.Context, stmtNode ast.StmtNode) (sqlexec.RecordSet, error) {
	...
	if execUseArbitrator {
		...
		if sg, ok := domain.GetDomain(s).InfoSchema().ResourceGroupByName(pmodel.NewCIStr(sessVars.StmtCtx.ResourceGroupName)); ok {
			switch sg.Priority {
			case pmodel.LowPriorityValue:
				memPriority = memory.ArbitrationPriorityLow
			case pmodel.MediumPriorityValue:
				memPriority = memory.ArbitrationPriorityMedium
			case pmodel.HighPriorityValue:
