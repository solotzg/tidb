...
func (e *IndexLookUpJoin) newInnerWorker(taskCh chan *lookUpJoinTask) *innerWorker {
	...
	iw := &innerWorker{
		...
		taskCh:        taskCh,
		ctx:           e.Ctx(),
		executorChk:   e.AllocPool.Alloc(e.InnerCtx.RowTypes, indexLookupJoinChunkInitCap(e.InitCap(), e.MaxChunkSize()), e.MaxChunkSize()),
...
func (ow *outerWorker) buildTask(ctx context.Context) (*lookUpJoinTask, error) {
	...
	for requiredRows > task.outerResult.Len() {
		chk := ow.executor.NewChunkWithCapacity(ow.OuterCtx.RowTypes, indexLookupJoinChunkInitCap(ow.executor.InitCap(), maxChunkSize), maxChunkSize)
