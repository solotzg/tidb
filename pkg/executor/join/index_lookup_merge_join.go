...
func (e *IndexLookUpMergeJoin) startWorkers(ctx context.Context) {
	...
	for i := 0; i < concurrency; i++ {
		...
		for j := 0; j < numResChkHold; j++ {
			e.joinChkResourceCh[i] <- e.NewChunkWithCapacity(e.RetFieldTypes(), indexLookupJoinChunkInitCap(e.InitCap(), e.MaxChunkSize()), e.MaxChunkSize())
		}
	}
...
func (imw *innerMergeWorker) fetchNextInnerResult(ctx context.Context, task *lookUpMergeJoinTask) (beginRow chunk.Row, err error) {
	task.innerResult = imw.innerExec.NewChunkWithCapacity(imw.innerExec.RetFieldTypes(), indexLookupJoinChunkInitCap(imw.innerExec.InitCap(), imw.innerExec.MaxChunkSize()), imw.innerExec.MaxChunkSize())
