...
// 4. Reads NEXT_TIME from mysql.tidb_mlog_purge.
func (*serverHelper) PurgeMVLog(ctx context.Context, sysSessionPool basic.SessionPool, mvLogID int64) (nextPurge time.Time, err error) {
	const (
		...
		findNextTimeSQL = `SELECT UNIX_TIMESTAMP(NEXT_TIME) FROM mysql.tidb_mlog_purge WHERE MLOG_ID = %? AND NEXT_TIME IS NOT NULL`
	)
	var (
