...
def main():
    ...
    parser.add_argument('--port', help="port", default=4077)
...
class Runner:
    ...
    def handle_tpch_run(self, mode, database, warehouses, threads, count=22):
        cmd_run = "tiup bench tpch --sf {warehouses} run --host {host} --port {port}  --db {db} --threads {threads} --count {count} --ignore-error".format(
    ...
    def restart_tikv(self):
        cmd = "tiup cluster restart tzg-test -R tikv -y"
